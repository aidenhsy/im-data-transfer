    ...
    while (hasMoreData) {
        ...
        for (const procurementOrder of procurementOrders) {
            ...
            for (const detail of procurementOrder.supplier_order_details) {
                ...
                detailMap.set(`${procurementOrder.id}-${detail.supplier_reference_id}`, {
                    procurementOrder,
        ...
        if (procurementUpdates.length > 0 || orderUpdates.length > 0) {
            await Promise.all([
                ...
                procurement.$transaction(procurementUpdates.map((update) => procurement.supplier_order_details.update(update))),
                order.$transaction(orderUpdates.map((update) => order.procurement_order_details.update(update))),
            ]);
            console.log(`Updated ${procurementUpdates.length} procurement records and ${orderUpdates.length} order records`);
        }
