...
const SHOP_ID = 3;
// Inventory “final” snapshot time for 7/31 (your code used 21:00:00Z)
const FINAL_COUNT_AT = '2025-07-31T21:00:00.000000Z';
// Purchasing period: 7/1 00:00 -> 7/31 21:00
const PERIOD_START = '2025-06-30T21:00:00.000000Z'; // > this means 7/1 00:00 JST
const PERIOD_END = '2025-07-31T21:00:00.000000Z';

const LAST_COUNT_ID = '3724'; // you hard-coded this before
...
const run = async () => {
  ...
  // 1) Fetch everything up front (minimal includes)
  const [shop, lastCount, finalCount, orderDetails, wac] = await Promise.all([
    ...
    imInventory.inventory_count.findFirst({
      where: { shop_id: SHOP_ID, created_at: FINAL_COUNT_AT },
      select: {
        id: true,
        created_at: true,
        status: true,
    ...
    imProcurement.supplier_order_details.findMany({
      where: {
        supplier_orders: {
          status: { in: [4, 5] },
          shop_id: SHOP_ID,
          receive_time: { gt: PERIOD_START, lt: PERIOD_END },
        },
      },
